...
class FastSignalTracker:
    ...
    async def close_signals_for_removed_symbols(self, removed_symbols: List[str]):
        """
        Close all open signals for symbols that were removed from universe
        
        Args:
            removed_symbols: List of symbols removed from universe
        """
        try:
            ...
            logger.info(
                f"🔄 [FastSignalTracker] Checking for open signals in {len(removed_symbols)} removed symbols"
            )
            
            with db_manager.get_session() as session:
                # Find all open signals for removed symbols
                open_signals = session.query(Signal).filter(
                    Signal.status == 'OPEN',
                    Signal.symbol.in_(removed_symbols)
                ).all()
                
                if not open_signals:
                    logger.info("✅ [FastSignalTracker] No open signals for removed symbols")
                    return
                
                logger.warning(
                    f"⚠️ [FastSignalTracker] Found {len(open_signals)} open signals for removed symbols, closing..."
                )
                
                for signal in open_signals:
                    symbol = signal.symbol
                    
                    # Try to get last known price from Redis (may still be cached)
                    price_data = redis_manager.get(f'price:{symbol}')
                    if price_data:
                        exit_price = float(price_data.get('mid', signal.entry_price))
                    else:
                        # Fallback to entry price if no data available
                        exit_price = float(signal.entry_price)
                        logger.warning(
                            f"⚠️ [FastSignalTracker] No price data for {symbol}, using entry price"
                        )
                    ...
                    # Calculate hold time
                    hold_time = int(
                        (datetime.now() - signal.created_at).total_seconds() / 60
                    )
                    ...
                    logger.info(
                        f"🏁 [FastSignalTracker] Closed {symbol} {signal.direction} "
                        f"(UNIVERSE_REMOVED) PnL: {pnl_percent:+.2f}% Hold: {hold_time}min"
                    )
                    ...
                    # Remove from cache
                    if signal.id in self.open_signals_cache:
                        del self.open_signals_cache[signal.id]
                
                # Commit all changes
                session.commit()
                logger.info(
                    f"✅ [FastSignalTracker] Closed {len(open_signals)} signals for removed symbols"
                )
                
        except Exception as e:
            logger.error(f"❌ [FastSignalTracker] Error closing signals for removed symbols: {e}")
